...
export default class Form extends Component {
  ...
  dateChange(event, date) {
    ...
    var value = new Date((date.includes(":")) ? mainDate.toDateString() + " " + date + ":00Z" : this.convertToMillis(date) + " " + mainDate.toLocaleTimeString() + "Z");
    value = new Date(value.setMinutes(value.getMinutes()+value.getTimezoneOffset()));
    //alert(value);
  ...
  render() {
    return (
      <form onSubmit={this.submitClient} id="breaking-form">
        ...
        <MuiPickersUtilsProvider utils={DateFnsUtils} locale={ru} ampm={false}>
          <KeyboardTimePicker
            margin="normal"
            ampm={false}
            openTo="hours"
